...
server.register(fastifyMongodb, {
  url: process.env.MONGODB_URI || 'mongodb://localhost:27017/documents',
  forceClose: true,
});
...
server.register(collaborationRoutes, { prefix: '/api/collaboration' });
...
const start = async () => {
  try {
    ...
    console.log(`Document service is running on ${host}:${port}`);
  } catch (err) {
